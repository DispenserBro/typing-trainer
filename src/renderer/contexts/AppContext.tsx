...
import type {
  AppExtensionsContextValue,
  AppGameContextValue,
  AppNavigationContextValue,
  AppPracticeContextValue,
  AppSettingsContextValue,
  AppStatsContextValue,
  AppUiContextValue,
  SaveHistory,
...
import {
  createEmptyModState,
  emitModEvent as emitModRuntimeEventFromState,
} from '../../core/addons/modApi';
import type {
  ModAPIState,
} from '../../core/addons/modApi';
import { normalizeMotivationProgress } from '../../core/motivation/progress';
import { importInterfaceLocaleFromPo, resolveRuntimeTranslation } from '../../core/i18n';
...
export function AppProvider({ children }: { children: ReactNode }) {
  ...
  const [modRuleOverrides, setModRuleOverrides] = useState<Map<ModRuleId, ModRuleValue>>(new Map());
  ...
  const modEventStateRef = useRef<ModAPIState>(createEmptyModState());
  ...
  const emitModRuntimeEvent = useCallback(<K extends ModEventName>(
    event: K,
    payload: ModEventPayloadMap[K],
  ) => {
    emitModRuntimeEventFromState(modEventStateRef.current, event, payload);
  }, []);
  ...
  const applyModEffects = useCallback(async (mods: InstalledMod[], addons: InstalledAddon[]) => {
    ...
    modEventStateRef.current = modEffects.state;
  ...
  const switchMode = useCallback((mode: string) => {
    ...
    emitModRuntimeEvent('modeSwitch', {
  ...
  const saveSetting = useCallback(<K extends keyof UserSettings>(key: K, val: UserSettings[K]) => {
    ...
    commitResolvedProgress(prev => ({ ...prev, settings: nextSettings }));
    emitModRuntimeEvent('settingChange', {
      scope: 'user',
      key,
      previousValue,
      nextValue: val,
    });
  }, [commitResolvedProgress, emitModRuntimeEvent]);

  const savePracticeSetting = useCallback(<K extends keyof PracticeSettings>(key: K, val: PracticeSettings[K]) => {
    const previousValue = practiceSettingsRef.current[key];
    if (previousValue === val) return;
  ...
  const saveHistory = useCallback<SaveHistory>((mode, wpm, acc, extras) => {
    const entry = persistHistory(mode, wpm, acc, extras);
    emitModRuntimeEvent('sessionFinish', {
      layoutId: currentLayout,
      entry,
    });
  }, [currentLayout, emitModRuntimeEvent, persistHistory]);

  const {
    saveGameState: persistGameState,
    grantGameItem,
    equipGameItem,
    unequipGameItem,
    ...
    onGameStateChange: (previousState, nextState) => {
      emitModRuntimeEvent('gameStateChange', {
        previousState,
        nextState,
      });
    },
  }), [currentLayout, emitModRuntimeEvent, layouts, mergedAchievementCatalog]);
  ...
  const importConfig = useCallback(async (): Promise<string | null> => {
    ...
    return 'settings.cards.presets.importErrors.unknownConfigType';
  ...
  const importInterfaceLocale = useCallback(async (): Promise<string | null> => {
    const imported = await window.api.importFile({
      title: resolveRuntimeTranslation(
        'settings.cards.language.importPoDialogTitle',
        undefined,
  ...
  const practiceValue = useMemo<AppPracticeContextValue>(() => ({
    ...
    saveHistory,
    unlockAchievements,
    emitModEvent: emitModRuntimeEvent,
  }), [
    layouts,
    allWords,
    ngramModel,
