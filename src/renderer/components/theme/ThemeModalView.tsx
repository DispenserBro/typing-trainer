...
type ThemeColorFieldDefinition = {
...
type ThemeModalViewProps = {
  ...
  onDelete: () => void;
  onExport: () => void;
  onLoadExisting: (themeName: string) => void;
  onSave: () => void;
  setColor: (key: keyof CustomThemeColors, value: string) => void;
  setName: (name: string) => void;
};

const COLOR_FIELDS: ThemeColorFieldDefinition[] = [
  { key: 'bg', labelKey: 'themeEditor.colors.bg' },
  { key: 'surface', labelKey: 'themeEditor.colors.surface' },
  { key: 'surface2', labelKey: 'themeEditor.colors.surface2' },
  { key: 'text', labelKey: 'themeEditor.colors.text' },
  { key: 'subtext', labelKey: 'themeEditor.colors.subtext' },
  { key: 'accent', labelKey: 'themeEditor.colors.accent' },
  { key: 'green', labelKey: 'themeEditor.colors.green' },
  { key: 'red', labelKey: 'themeEditor.colors.red' },
  { key: 'yellow', labelKey: 'themeEditor.colors.yellow' },
];
    ...
    <ModalLayout
      ...
      footer={(
        <ActionRow stretch className="modal-actions">
          <Button variant="accent" onClick={onSave}>{t('common.actions.save')}</Button>
          <Button onClick={onExport} disabled={!name.trim()}>{t('common.actions.export')}</Button>
    ...
    >
      {customThemeNames.length > 0 && (
        <div className="theme-existing-block">
          <span className="card-desc">{t('themeEditor.loadExisting')}</span>
          <ActionRow className="theme-existing-actions">
            {customThemeNames.map(themeName => (
              <Button
                key={themeName}
                className="theme-existing-button"
                onClick={() => onLoadExisting(themeName)}
              >
                {themeName}
              </Button>
            ))}
      ...
      <div className="color-grid">
        {COLOR_FIELDS.map(field => (
          <ThemeColorField
            ...
            onChange={(nextValue) => setColor(field.key, nextValue)}
          />
        ))}
      </div>

      <h4 className="theme-section-title">{t('themeEditor.typography.title')}</h4>
      <div className="theme-ext-grid">
        <label className="theme-ext-field">
          <span>{t('themeEditor.typography.fontSans')}</span>
          <TextInput
            value={colors.fontSans ?? ''}
            onChange={event => setColor('fontSans', event.target.value || '')}
            placeholder="Inter, Segoe UI, sans-serif"
          />
        </label>
        <label className="theme-ext-field">
          <span>{t('themeEditor.typography.fontMono')}</span>
          <TextInput
            value={colors.fontMono ?? ''}
            onChange={event => setColor('fontMono', event.target.value || '')}
            placeholder="JetBrains Mono, Consolas"
          />
        </label>
        <label className="theme-ext-field">
          <span>{t('themeEditor.typography.radius')}</span>
          <TextInput
            value={colors.radius ?? ''}
            onChange={event => setColor('radius', event.target.value || '')}
            placeholder="10px"
          />
        </label>
        <label className="theme-ext-field">
          <span>{t('themeEditor.typography.radiusSm')}</span>
          <TextInput
            value={colors.radiusSm ?? ''}
            onChange={event => setColor('radiusSm', event.target.value || '')}
            placeholder="6px"
          />
        </label>
        <label className="theme-ext-field">
          <span>{t('themeEditor.typography.transitionSpeed')}</span>
          <TextInput
            value={colors.transitionSpeed ?? ''}
            onChange={event => setColor('transitionSpeed', event.target.value || '')}
            placeholder="0.2s"
          />
        </label>
      </div>
